...
```{r, include = FALSE }
# included all the libraries used in report
library(dplyr)
library(date)
library(knitr)  
library(lubridate)
...
```{r}
# Using relative path for importing data
powerplant<-read_csv('./Data/global_power_plant_database.csv')
...
# Creating new dataframe that we use for coming code chunks
...
```{r}
# Converting years columns into single column and their values to power generation
powerplant_new1%>%
  ...
  filter(!is.na(power_generation))->powerplant_new1
```
...
# Some visualisations for statistical analysis
```{r}
powerplant_new%>%
